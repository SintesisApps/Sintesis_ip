...
var _this=this
_this.touches={};_this.positions={current:0};_this.id=(new Date()).getTime();_this.container=(selector.nodeType)?selector:dQ(selector)[0];_this.times={};_this.isTouched=false;_this.realIndex=0;_this.activeSlide=0;_this.previousSlide=null;_this.langDirection=window.getComputedStyle(_this.container,null).getPropertyValue('direction')
_this.support={touch:_this.isSupportTouch(),threeD:_this.isSupport3D()}
_this.use3D=_this.support.threeD;var defaults={mode:'horizontal',ratio:1,speed:300,freeMode:false,freeModeFluid:false,slidesPerSlide:1,slidesPerGroup:1,simulateTouch:true,followFinger:true,shortSwipes:true,moveStartThreshold:false,autoPlay:false,onlyExternal:false,createPagination:true,pagination:false,resistance:true,scrollContainer:false,preventLinks:true,initialSlide:0,keyboardControl:false,mousewheelControl:false,resizeEvent:'auto',useCSS3Transforms:true,slideElement:'div',slideClass:'swiper-slide',wrapperClass:'swiper-wrapper',paginationClass:'swiper-pagination-switch',paginationActiveClass:'swiper-active-switch'}
...
el.isActive=function(){if(el.index()==_this.activeSlide)return true;else return false;}
...
_this.insertSlideAfter=function(index,html,slideClassList,el){if(!index)return false;if(index.nodeType){return _this._extendSwiperSlide(index).insertAfter(index)}
...
_this.currentSlide=function(){return _this.slides[_this.activeSlide]}
...
var newWidth=parseInt(_width,10);var newHeight=parseInt(_height,10);if(isNaN(newWidth)||_this.ie8&&(_width.indexOf('%')>0)){newWidth=_this.container.offsetWidth- parseInt(window.getComputedStyle(_this.container,null).getPropertyValue('padding-left'),10)- parseInt(window.getComputedStyle(_this.container,null).getPropertyValue('padding-right'),10)}
if(isNaN(newHeight)||_this.ie8&&(_height.indexOf('%')>0)){newHeight=_this.container.offsetHeight- parseInt(window.getComputedStyle(_this.container,null).getPropertyValue('padding-top'),10)- parseInt(window.getComputedStyle(_this.container,null).getPropertyValue('padding-bottom'),10)}
...
wrapper.style.width=wrapperWidth+"px";wrapper.style.height=wrapperHeight+"px";if(params.initialSlide>0&&params.initialSlide<numOfSlides&&!firstInit){_this.realIndex=_this.activeSlide=params.initialSlide;if(params.loop){_this.activeSlide=_this.realIndex-params.slidesPerSlide}
if(isHorizontal){_this.positions.current=-params.initialSlide*slideWidth;_this.setTransform(_this.positions.current,0,0);}
...
_this.isTouched=true;isTouchEvent=event.type=='touchstart';if(!isTouchEvent||event.targetTouches.length==1){_this.callPlugins('onTouchStartBegin');if(params.loop)_this.fixLoop();if(!isTouchEvent){if(event.preventDefault)event.preventDefault();else event.returnValue=false;}
...
return false}}
function onTouchEnd(event){if(params.onlyExternal||!_this.isTouched)return
...
slideChangeCallbacks()
return true}
function slideChangeCallbacks(){_this.callPlugins('onSlideChangeStart');if(params.onSlideChangeStart){params.onSlideChangeStart(_this)}
if(params.onSlideChangeEnd){_this.transitionEnd(params.onSlideChangeEnd)}}
_this.updateActiveSlide=function(position){_this.previousSlide=_this.realIndex
_this.realIndex=Math.round(-position/slideSize)
if(!params.loop)_this.activeSlide=_this.realIndex;else{_this.activeSlide=_this.realIndex-params.slidesPerSlide
if(_this.activeSlide>=numOfSlides-params.slidesPerSlide*2){_this.activeSlide=numOfSlides- params.slidesPerSlide*2- _this.activeSlide}
if(_this.activeSlide<0){_this.activeSlide=numOfSlides- params.slidesPerSlide*2+ _this.activeSlide}}
if(_this.realIndex==numOfSlides)_this.realIndex=numOfSlides-1
if(_this.realIndex<0)_this.realIndex=0
if(params.pagination){_this.updatePagination()}}
...
else{if(axis=='x')curTransform=parseInt(el.style.left,10)||0
