Swiper.prototype.plugins.tdFlow=function(swiper,params){if(!swiper.support.threeD)return;var slides,wrapperSize,slideSize,initialized;var isH=swiper.params.mode=='horizontal';var defaults={rotate:50,stretch:0,depth:100,modifier:1,shadows:true}
params=params||{};for(var prop in defaults){if(!(prop in params)){params[prop]=defaults[prop]}}
function setTransition(el,dur){var es=el.style
...
function setTransform(el,transform){var es=el.style
es.webkitTransform=es.MsTransform=es.msTransform=es.MozTransform=es.OTransform=es.transform=transform}
function init(){initialized=true;slides=swiper.slides
for(var i=0;i<slides.length;i++){setTransition(slides[i],0)}
if(isH){wrapperSize=swiper.wrapper.offsetWidth;slideSize=wrapperSize/slides.length;for(var i=0;i<slides.length;i++){slides[i].swiperSlideOffset=slides[i].offsetLeft}}
else{wrapperSize=swiper.wrapper.offsetHeight;slideSize=wrapperSize/slides.length;for(var i=0;i<slides.length;i++){slides[i].swiperSlideOffset=slides[i].offsetTop}}}
...
var hooks={onFirstInit:function(args){slides=swiper.slides
if(params.shadows){var shadowEl1=document.createElement('div')
...
init();threeDSlides({x:swiper.getTranslate('x'),y:swiper.getTranslate('y'),z:swiper.getTranslate('z')});},onInit:function(args){init();threeDSlides({x:swiper.getTranslate('x'),y:swiper.getTranslate('y'),z:swiper.getTranslate('z')});},onSetTransform:function(transform){threeDSlides(transform);},onSetTransition:function(args){for(var i=0;i<swiper.slides.length;i++){setTransition(swiper.slides[i],args.duration)
if(isH&&params.shadows){setTransition(swiper.slides[i].querySelector('.swiper-slide-shadow-left'),args.duration)
setTransition(swiper.slides[i].querySelector('.swiper-slide-shadow-right'),args.duration)}
else if(params.shadows){setTransition(swiper.slides[i].querySelector('.swiper-slide-shadow-top'),args.duration)
